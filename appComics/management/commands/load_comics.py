  ...
  },
  {
    ...
    "foto": 'victoria.jpg'
        ...
        for comic_data in data:
            # Maneja el caso 'Sin "stock"'
            if comic_data['stock'] == 'Sin "stock"':
                comic_data['stock'] = 0  # O cualquier otro valor predeterminado

            # Asegúrate de que el precio sea un número decimal
            comic_data['precio'] = float(comic_data['precio'])

            # Genera un slug único
            base_slug = slugify(comic_data['nombre'])
            slug = base_slug
            counter = 1
